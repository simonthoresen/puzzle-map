...
const ans = N.map(n => String.fromCharCode(n + 64)).join('');

root.innerHTML =
  '<div style="color:#888;font-size:min(4vw,1rem)">A=1, B=2, C=3 \u2026 Z=26</div>' +
  '<div style="color:#0f0;font:bold min(6vw,1.6rem) monospace;letter-spacing:.3em">' + N.join(' \u2014 ') + '</div>' +
  '<input id="inp" style="font:min(5vw,1.5rem) monospace;padding:.5rem;background:#222;color:#0f0;border:2px solid #444;border-radius:8px;text-align:center;width:80vw;text-transform:uppercase;margin-top:.5rem" autocomplete="off" autocapitalize="characters" autocorrect="off" placeholder="Decoded word">' +
